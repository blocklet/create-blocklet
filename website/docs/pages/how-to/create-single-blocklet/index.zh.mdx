...
程序会自动创建好对应模板的目录结构，根据其提示的信息进行依赖的安装即可进入项目的开发模式（前提在本机中有安装 blocklet-server 环境）。
...
```
通过 `create-blocklet` 工具创建出来的项目，已经自动配置好了npm script，你只需要执行 `npm run bundle` 即可将你的 blocklet 打包

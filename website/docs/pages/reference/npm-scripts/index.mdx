...
The project created by the `create-blocklet` tool will have the corresponding `npm script` configured in `package.json` for developers to use.

The `npm script` can theoretically be executed using the `npm`, `yarn`, and `pnpm` commands.
